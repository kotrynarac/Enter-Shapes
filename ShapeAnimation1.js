...
function buildParticles() {
  ...
  const maxEdge = max(particles.map(p => p.edgeFactor));
...
function draw() {
  ...
  let orbitRadius = min(width, height) * 0.32 * currentShapeScale;

  let spotX = width * 0.5 + cos(angle) * orbitRadius;
  let spotY = height * 0.5 + sin(angle) * orbitRadius;

  // 🔥 Enhanced spotlight radius for tiny screens
  let tinyClamp = map(width, 75, 400, 0.8, 1, true); // increase min radius
  let scaledRadius = COLOR_RADIUS * currentShapeScale * tinyClamp;

  for (let p of particles) {

    let microScale = map(width, 75, 400, 0.15, 1, true);
    ...
    let a = atan2(p.baseY - height * 0.5, p.baseX - width * 0.5);
...
function extractPoints(img) {
  ...
  currentShapeScale =
    min(width / img.width, height / img.height) * HERO_SCALE;
...
function generateCodePoints() {
  ...
  let cols = max(ceil(width / (BASE_CHAR_WIDTH * currentShapeScale)), 1);
  let rows = max(ceil(height / (BASE_LINE_HEIGHT * currentShapeScale)), 1);
...
// function preload() {
//   font = loadFont("Wix Madefor Text VF-normal-400-100.ttf");
//   shapeImg = loadImage("Enter14.svg");
// }
...
//   let tinyClamp = map(width, 75, 400, 0.8, 1, true);
//   let scaledRadius = COLOR_RADIUS * currentShapeScale * tinyClamp;
...
// function keyPressed() {
//   if (key === 's' || key === 'S') {
//     saveCanvas(canvasRef, 'hero-frame-transparent', 'png');
//   }
// }
